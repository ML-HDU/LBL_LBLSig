...
class LBL(ABC):
    ...
    def __init__(self, loss_function: str, net_name: str):
        ...
        assert loss_function in ('LBL', ), 'Loss function must be "LBL".'
    ...
    def train(self, train_dataset, optimizer_parameters: My_Optimizer_Parameters, mu,
              warm_up_n_epochs):
        ...
        for k_epoch in range(optimizer_parameters.number_of_epochs):
            ...
            for k_batch_index, k_train_dataloader in enumerate(train_dataloader):
                ...
                if k_epoch >= self.warm_up_n_epochs:
                    ...
                    self.R_square = 2 * distance_errors.max()
                    self.R_square = self.R_square.detach()  # ！！！！！非常重要！！！！！

                    loss = -torch.mean(torch.log(self.R_square - distance_errors))
